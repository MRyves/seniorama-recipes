...
export class RecipeService {
  ...
  constructor(private ngf: AngularFirestore, private toast: ToastrService, private router: Router) {
  }
  ...
  private saveNewRecipe(recipe: RecipeModel) {
    return this.recipeCollection.add(recipe)
      .then((docRef) => this.toast.success(`<a>${recipe.name}</a>`, 'Neues Rezept erstellt!', {
        enableHtml: true
  ...
  private saveRecipe(recipe: RecipeModel) {
    return this.recipeCollection.doc(recipe.uid).update(recipe)
      .then(() => this.toast.success(`<a>${recipe.name}</a>`, 'Erfolgreich angepasst!', {enableHtml: true})
        .onTap.pipe(
          take(1)
        ).subscribe(() => {
